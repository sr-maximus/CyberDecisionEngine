...
def _completed_run(run_id: str, domains: list[str], **request_overrides: object) -> RunRecord:
    request = DomainAnalysisRequest(
        domains=domains,
        organization_name="Frontera Energy Corporation",
        legal_name="Frontera Energy Corporation",
        sector="Energy infrastructure",
        subsector="Ports, pipelines and logistics",
        country="Canada",
        brands=["Frontera Energy", "Puerto Bahia", "ODL"],
        strategic_assets=["Puerto Bahia", "ODL pipeline"],
...
def test_exact_domain_scope_reuses_only_missing_declared_profile_fields():
    domains = ["fronteraenergy.ca", "puertobahia.com.co", "odl.com.co"]
    ...
    assert hydrated.organization_name == "Frontera Energy Corporation"
    assert hydrated.sector == "Energy infrastructure"
    assert hydrated.country == "Colombia"
    assert hydrated.brands == ["Frontera Energy", "Puerto Bahia", "ODL"]
